...
class Instagram {
  ...
  public function __construct($config) {
    if (true === is_array($config)) {
      ...
      $this->setCallback($config['redirect_url']);
  ...
  public function getLoginUrl($scope = array('basic')) {
    if (is_array($scope) && count(array_intersect($scope, $this->_scopes)) === count($scope)) {
      return self::API_OAUTH_URL . '?client_id=' . $this->getClientID() . '&redirect_uri=' . $this->getCallback() . '&scope=' . implode('+', $scope) . '&response_type=code';
    } else {
  ...
  protected function _makeCall($function, $auth = false, $params = null, $method = 'GET') {
    if (false === $auth) {
      ...
      $authMethod = '?client_id=' . $this->getClientID();
    } else {
   ...
   * @return void
   */
  public function setCallback($redirect_url) {
    $this->_callbackurl = $redirect_url;
  }
